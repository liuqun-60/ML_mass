...
def plot_raw_data_distributions(
        df: pd.DataFrame,
        max_sample: int = 15000,
        n_bins: int = 20,
) -> Optional[go.Figure]:
    """原始数据关键字段分布（抽样 + 预聚合，适合大文件快速预览）"""
    if df is None or len(df) == 0:
        return None

    panel_defs = [
        ('speed_kmh', '车速 (km/h)', '#3B82F6', 10.0, False),
        ('acceleration_x', '加速度 (m/s²)', '#10B981', 1.0, False),
        ('force_n', '牵引力 (N)', '#F97316', 2000.0, False),
        ('mass_kg', '标注质量 (kg)', '#8B5CF6', None, True),
    ]
    if 'force_n' not in df.columns and 'motor_torque_nm' in df.columns:
        panel_defs[2] = ('motor_torque_nm', '电机扭矩 (Nm)', '#F97316', None, False)

    cols = [p[0] for p in panel_defs if p[0] in df.columns]
    if not cols:
        return None

    n_rows = len(df)
    use_sample = n_rows > max_sample
    if use_sample:
    ...
    panels = []
    for col, label, color, threshold, use_full_counts in panel_defs:
        ...
        if use_full_counts:
            ...
            panels.append(('counts', counts.index.values, counts.values, label, color, threshold))
        else:
            series = pd.to_numeric(sub[col], errors='coerce').dropna()
            if len(series) == 0:
                continue
            panels.append(('hist', series.to_numpy(dtype=np.float64), None, label, color, threshold))
    ...
    n_rows_plot = (n_panels + n_cols - 1) // n_cols
    ...
    for idx, panel in enumerate(panels):
        ...
        label, color, threshold = panel[3], panel[4], panel[5]

        if kind == 'counts':
            x_vals, y_vals = panel[1], panel[2]
            fig.add_trace(
                go.Bar(
            ...
            if threshold is not None:
                fig.add_vline(
                    ...
                    line_width=1.5, row=row, col=col_idx,
                )

    sample_note = f'（随机抽样 {max_sample:,}/{n_rows:,} 行）' if use_sample else ''
    fig.update_layout(
        title=dict(
            text=f'原始数据关键字段分布{sample_note}',
            x=0.5,
            xanchor='center',
            y=0.98,
            yanchor='top',
